...
// API key
const APIkey = "7c052de08e8eb011fa67c53cae53ea12";
...
const App = () => {
  ...
  if (!data) {
    return <div className=' h-screen w-full flex bg-gradientBg bg-no-repeat bg-cover bg-center flex-col justify-center items-center text-center'>
      <div>
  ...
  return <div className="w-full h-screen bg-gradientBg bg-no-repeat bg-cover bg-center flex flex-col items-center justify-center px-4 lg:px-0" >
    {error && <div className='w-full max-w-[90vw] lg:max-w-[450px] bg-[#ff208c] text-white absolute top-2 lg:top-10 p-4 capitalize rounded-md'>{`${error.response.data.message}`}</div>}
          ...
          <div className='flex  justify-between'>
            ...
            <div className="flex items-center  gap-x-2">
              ...
              <div className='text-sm' >
                Wind <span className='ml-2 text-base'>{data.wind.speed} m/s</span>
              </div>
            </div>
          </div>
        </div>
